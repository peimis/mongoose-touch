...
void IRAM_ATTR disp_spi_transfer_cmd_data(int8_t cmd, uint8_t *data, uint32_t len) {
	...
	gpio_set_level(mgos_sys_config_get_ili9341_dc_pin(), 1);
...
// ================================================================
//----------------------------------------------------------------------------------------------
static void IRAM_ATTR _pushColorRep(color_t *color, uint32_t len, uint8_t rep, uint8_t wait)
{
	if (len == 0) return;
	if (!(disp_spi->cfg.flags & SPI_DEVICE_HALFDUPLEX)) return;
	...
	gpio_set_level(mgos_sys_config_get_ili9341_dc_pin(), 1);								// Set DC to 1 (data mode);
...
//-------------------------------------------------------------------------------------------
void IRAM_ATTR mgos_ili9341_pushColorRep(int x1, int y1, int x2, int y2, color_t color, uint32_t len)
{
...
void mgos_ili9341_display_init()
{
  ...
  mgos_ili9341_pushColorRep(0, 0, _width-1, _height-1, (color_t){0,0,0}, (uint32_t)(_height*_width));
}
